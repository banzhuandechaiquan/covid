<!--中国疫情地展示-->
<template>
  <div>
    <p class="title"><i></i> 疫情地图</p>
    <div id="chinaMap"></div>
  </div>
</template>

<script>
  export default {
    name: "Map",
    created() {
    },
    mounted() {
       // this.$charts.chinaMap('chinaMap')
      this.getMapData();
    },
    methods:{
      async getMapData() {
        await this.$axios({
          method:'get',
          url:"chinaMap.json",
        }).then(res => {
          // console.log(res.data);
          // console.log(url);
          const allCities = []
          // 各省市疫情数据|数据格式：{name:"内蒙古",value:10,itemStyle:{normal:{areaColor:"#f00"}}}
          for(let i = 0;i < res.data.data.list.length;i++){
            // console.log('hhh');
            const temp = {
              name:res.data.data.list[i].name,      //省市名称
              value:res.data.data.list[i].value,    //省市确诊人数
              // itemStyle:{                        //地图省市疫情颜色
              //   normal:{
              //     areaColor:this.setColor(res.data.data.list[i].value)
              //   }
              // }
            }
            allCities.push(temp)
          }
          this.$charts.chinaMap('chinaMap',allCities)
        }).catch(err => {
          console.log(err);})
      },
      // 根据省市疫情人数来呈现的省市疫情颜色
      // setColor(value) {
      //   let currentColor = "";
      //   switch (true) {
      //     case value > 0 && value < 10:
      //       currentColor = "#fff";
      //       break;
      //     case value > 10 && value < 99:
      //       currentColor = "#FDFDCF";
      //       break;
      //     case value > 100 && value < 999:
      //       currentColor = "#FE9E83";
      //       break;
      //     case value > 1000 && value < 9999:
      //       currentColor = "#E55A4E";
      //       break;
      //     case value >= 10000:
      //       currentColor = "#4F070D";
      //       break;
      //   }
      //   return currentColor;
      // }
    }
  }
</script>

<style scoped>
  #chinaMap {
    width: 100%;
    height: 400px;
  }
  .title{
    border-top: 0.005rem solid #ebebeb;
    border-bottom: 0;
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-align: center;
    -ms-flex-align: center;
    align-items: center;
    -webkit-box-pack: start;
    -ms-flex-pack: start;
    justify-content: flex-start;
    height: 0.44rem;
    padding: 0 0.16rem;
    font-weight: 500;
    font-size: 0.17rem;
    line-height: 0.44rem;
    background: #fff;
  }
  .title::before {
    content: '';
    width: 5px;
    height: 20px;
    background: #4169e2;
    margin-right: 10px;
  }
</style>